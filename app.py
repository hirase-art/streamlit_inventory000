...
with st.spinner('データを準備中...'):
    ...
    # 在庫は商品ID単位で集約
    df_inv_final = df_inv_raw[df_inv_raw['品質区分ID'].astype(str).isin(['1', '2'])].pivot_table(
        index='商品ID', columns='倉庫ID', values='利用可能', aggfunc='sum'
    ).fillna(0).rename(columns={'7': '大阪', '8': '千葉', 7: '大阪', 8: '千葉'})
    
    for col in ['大阪', '千葉']:
        if col not in df_inv_final.columns: df_inv_final[col] = 0
    df_inv_final['現在庫合計'] = df_inv_final['大阪'] + df_inv_final['千葉']
    df_inv_final = df_inv_final.reset_index()
...
sel_wh = st.sidebar.selectbox("出荷元倉庫:", wh_list)
biz_list = ["すべて"] + sorted(df_m_ship_raw['業務区分ID'].unique().tolist())
sel_biz = st.sidebar.selectbox("業務区分:", biz_list)
...
# 【重要：重複排除】商品IDが重複している場合、最初の1件だけを残す
df_m = df_m.drop_duplicates(subset=['商品ID'])
...
show_limit = st.sidebar.slider("表示期間 (列数):", 4, 24, 12)
avg_period = st.sidebar.slider("予測平均期間:", 1, 6, 3)
...
def display_analysis_table(df_ship_all, master, inv, incoming, title, period_label):
    ...
    # 【結合のコア】マスタ(master)は既に unique なので、ここで重複は発生しなくなります
    ...
    st.subheader(title)
    st.dataframe(
        res[base_cols + list(piv.columns[:show_limit])], 
