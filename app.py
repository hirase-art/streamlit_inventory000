...
def apply_filters(df, master, search_id_in, search_name_in, dai, chu, sho):
    ...
    # 3. ★商品名の補完 (KeyError: '商品名_m' を絶対に起こさない)
    # 元の df に「商品名」がなく、master から来た場合、列名は「商品名」になります
    # 元の df に「商品名」があり、master にもある場合、列名は「商品名_master」になります
